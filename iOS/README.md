...
- 准备通话环境。
    ...
    > **注意2：在每次发起呼叫前，我们建议调用 `prepareForCall` 方法来更新 roomId，以确保通话的安全性。<br><br>这里的 roomId 是主叫发起呼叫并进行通话时使用的 RTC 频道号。被叫在接收到呼叫后会加入该频道，通过每次更新 roomId 可以保证每次通话都使用不同的 RTC 频道，进而确保通话的私密性。**
- 呼叫
